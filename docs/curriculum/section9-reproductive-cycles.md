...
```mermaid
graph TD
    A[Developing Follicles release Estrogen] -->|Estrogen rises above 200 pg/mL for 36 hours| B[Switches from Negative to Positive Feedback]
    B -->|Stimulates Hypothalamus| C[GnRH pulses increase in frequency]
    C -->|Stimulates Anterior Pituitary| D[Massive release of LH & FSH]
    D -->|LH Surge| E[Follicle ruptures, releasing egg: Ovulation]
    E --> F[Ruptured follicle becomes Corpus Luteum, secreting Progesterone]
    F -->|Negative Feedback| A & B
```
...
```mermaid
graph TD
    A[FSH recruits cohort of follicles] --> B[Granulosa cells release Estrogen]
    B --> C[Estrogen builds endometrial lining: Proliferative phase]
    C --> D[Peak Estrogen triggers LH surge on Day 14]
    D --> E[Ovulation: Oocyte released]
    E --> F[Corpus Luteum secretes Progesterone]
    F --> G[Progesterone drives uterine gland secretions: Secretory phase]
    G -->|No fertilisation: Corpus Luteum degenerates| H[Progesterone drops: Menstruation occurs]
```
