...
```mermaid
graph TD
    A[GH binds GHR Monomers] -->|Induces| B[GHR Homodimerization]
    B -->|Activates| C[Associated JAK2 Kinases]
    C -->|Phosphorylates| D[JAK2 & GHR Tyrosine Residues]
    D -->|Recruits and Phosphorylates| E[STAT5 Transcription Factors]
    E -->|Dimerize & Translocate to| F[Cell Nucleus]
    F -->|Transcribes target genes| G[IGF-1 synthesis, cellular growth & metabolism]
```
...
graph TD
    ...
    Sleep[Deep Sleep / Exercise / Hypoglycemia] -->|Stimulates| Hypo[Hypothalamus]
    FFA[Elevated FFA / Hyperglycemia / Somatostatin] -->|Inhibits| Hypo
    
    %% Hypothalamic hormones
    Hypo -->|Releases GHRH| Ant_Pit[Anterior Pituitary Somatotropes]
    Hypo -.->|Releases Somatostatin - -| Ant_Pit
    
    %% Pituitary release
    Ant_Pit -->|Secretes GH| Blood[Circulating Blood]
    
    %% Target organs and effects
    Blood -->|Direct Metabolic Effects| Muscle[Skeletal Muscle: Insulin Resistance]
    Blood -->|Direct Metabolic Effects| Adipose[Adipose Tissue: Lipolysis HSL Activation]
    Blood -->|Acts on hepatocytes| Liver[Liver]
    
    %% IGF-1 synthesis and indirect effects
    Liver -->|Synthesizes and secretes| IGF1[IGF-1 / Somatomedin-C]
    IGF1 -->|Chondrocytes in Epiphyseal Plate| Bone[Linear Skeletal Growth]
    IGF1 -->|Protein Synthesis| Organs[Visceral & Muscle Hypertrophy]
    
    %% Feedback loops
    IGF1 -.->|Negative Feedback| Ant_Pit
    IGF1 -.->|Negative Feedback / Stimulates Somatostatin| Hypo
    Blood -.->|Negative Feedback| Hypo
```
