...
```mermaid
graph TD
    A[Sound Waves enter External Auditory Canal] --> B[Vibrate Tympanic Membrane]
    B --> C[Amplification by Ossicles: Malleus, Incus, Stapes]
    C --> D[Stapes pushes Oval Window, generating fluid waves in Cochlea]
    D --> E[Basilar Membrane vibrates, shearing hair cell stereocilia]
    E --> F[Tip links pull open K+ channels -> K+ influx from high-potential Endolymph]
    F --> G[Depolarization opens Voltage-Gated Ca2+ Channels]
    G --> H[Glutamate release activates Cochlear Nerve CN VIII]
```
