...
```mermaid
graph TD
    A[BP Drop] -->|Decreased Stretch| B[Carotid & Aortic Baroreceptors]
    B -->|Decreased Firing via CN IX/X| C[NTS in Medulla]
    C -->|Decreased Inhibition of| D[RVLM]
    D -->|Increased Sympathetic Outflow| E[Heart & Blood Vessels]
    E -->|Increased HR, Contractility, and Vasoconstriction| F[Blood Pressure Rises]
```
