...
```mermaid
graph TD
    A[ADH binds V2 receptors in Medullary Collecting Duct] -->|Inserts| B[UT-A1 & UT-A3 Urea Transporters]
    B -->|Urea diffuses into| C[Inner Medullary Interstitium]
    C -->|Contributes 50% of| D[Hyperosmotic Medullary Gradient]
    C -->|Urea enters via UT-2 into| E[Thin Limbs of Loop of Henle]
    E -->|Flows up to| F[Distal Nephron and back to Collecting Duct]
    F -->|Recycles back to| B
```
...
```mermaid
graph TD
    %% Engine of the Multiplier
    TAL[Thick Ascending Limb] -->|Impermeable to Water| TAL_H2O[Water Retained in Lumen]
    TAL -->|Active transport via NKCC2| Pump_NaCl[NaCl pumped into Medullary Interstitium]
    
    %% The Single Effect
    Pump_NaCl -->|Raises| Interstitium_Osm[Interstitial Osmolarity]
    Interstitium_Osm -->|Draws water out of| tDLH[Thin Descending Limb]
    tDLH -->|Equilibrates| Conc_Fluid[Tubular Fluid Concentrated at Loop Tip]
    Conc_Fluid -->|Flows forward into| TAL
    
    %% Loop of Multiplication
    TAL_H2O -->|Leaves fluid| Hypo_Fluid[Hypotonic Fluid 100 mOsm/L enters DCT]
    
    %% Vasa Recta Exchange
    VR_Desc[Descending Vasa Recta] -->|Solutes enter, Water leaves| VR_Tip[Hyperosmotic Blood at Tip]
    VR_Tip -->|Ascending Vasa Recta| VR_Asc[Solutes leave, Water enters]
    VR_Asc -->|Preserves| Interstitium_Osm
    
    %% Final Concentration
    Interstitium_Osm -.->|Osmotic pull on| CD[Collecting Duct]
    ADH[Antidiuretic Hormone] -->|Inserts AQP2 in| CD
    CD -->|Water reabsorbed into Interstitium| Conc_Urine[Excretion of Concentrated Urine]
```
