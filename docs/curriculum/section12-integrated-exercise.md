...
```mermaid
graph TD
    Heat[Heat Exposure & Exercise] -->|Triggers| Sweat[Increased Sweat Output up to 3 L/hour]
    Sweat -->|Causes| Dehydration[Mild volume depletion]
    Dehydration -->|Activates| RAAS[Renin-Angiotensin-Aldosterone System]
    RAAS -->|Aldosterone release| Adrenal[Adrenal Cortex]
    Adrenal -->|Aldosterone acts on| Duct[Sweat Gland Reabsorptive Ducts]
    Duct -->|Reabsorbs| NaCl[Sodium & Chloride ions conserved]
    NaCl -->|Result| Dilute[Dilute sweat prevents salt depletion]
```
...
```mermaid
graph TD
    A[Strenuous Exercise starts] --> B[Sympathetic Nervous System fires]
    B --> C[Vasoconstriction of Renal & GI vessels / Vasodilation of Coronary & Muscle vessels]
    B --> D[Heart Rate & Stroke Volume increase, raising Cardiac Output]
    B --> E[Diaphragm & accessory muscles contract faster, raising Ventilation]
    E --> F[Arterial blood gases remain normal]
    D & F --> G[Muscles receive oxygen and nutrients to maintain contraction]
```
