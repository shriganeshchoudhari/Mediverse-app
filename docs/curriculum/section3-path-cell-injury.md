...
```mermaid
graph TD
    A[Ischemia / Hypoxia] --> B[ATP Depletion]
    B --> C[Failure of Na+/K+ Pump]
    B --> D[Failure of Ca2+ Pump]
    C --> E[Cellular Swelling & Blebbing]
    D --> F[Calcium Influx]
    F --> G[Enzyme Activation: Proteases & Phospholipases]
    G --> H[Membrane & DNA Rupture - Irreversible]
```
