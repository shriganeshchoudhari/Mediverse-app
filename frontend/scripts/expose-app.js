...
function startDevtunnel() {
  console.log('\n\x1b[32m🚀 Starting Microsoft Dev Tunnel on port ' + FRONTEND_PORT + ' (Public Access)...\x1b[0m');
  const dt = spawn('devtunnel', ['host', '-p', String(FRONTEND_PORT), '--allow-anonymous'], {
    shell: true,
    stdio: 'inherit'
  });
  dt.on('error', (err) => console.error('Dev Tunnel error:', err));
}
...
function main() {
  ...
  console.log('  [1] \x1b[32mMicrosoft Dev Tunnels (devtunnel)\x1b[0m (Recommended — Official Microsoft Tunnel, Public Access, Fast)');
  console.log('  [2] \x1b[36mCloudflare Quick Tunnel\x1b[0m (Free HTTPS + WebSockets, No signup)');
  console.log('  [3] \x1b[34mLocaltunnel\x1b[0m (Instant zero-install public URL via npx)');
  console.log('  [4] \x1b[35mNgrok\x1b[0m (With web inspector @ http://localhost:4040)');
  console.log('  [5] \x1b[33mLocal Network / Wi-Fi Sharing\x1b[0m (Access on physical phone on same Wi-Fi)');
  console.log('  [6] Exit\n');
  ...
  rl.question('\x1b[1mEnter choice [1-6] (default 1): \x1b[0m', (choice) => {
    rl.close();
    ...
    switch (opt) {
      ...
      case '6':
        ...
        process.exit(0);
      default:
        console.log('Invalid option. Defaulting to Microsoft Dev Tunnel...');
        startDevtunnel();
        break;
    }
  });
