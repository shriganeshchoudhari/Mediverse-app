...
export default function TransfusionLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("transfusion-abo-rh-antigens-coombs-testing");

  const activeModule = getTransfusionModuleById(selectedModuleId) || TRANSFUSION_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/20 text-red-300 border border-red-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Transfusion Medicine &amp; Immunohematology Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🧪 Transfusion Medicine (TRANS-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              ABO/Rh Molecular Genetics &amp; Bombay Phenotype ($hh$), Direct/Indirect Coombs Crossmatching, Targeted Component Dosing &amp; Balanced 1:1:1 Massive Transfusion Protocol, TRALI vs TACO Differential Diagnosis, and Rh Alloimmunization Kleihauer-Betke Calculator.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Transfusion Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <TransfusionLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-red-950/40 border border-red-800/60">
                <div className="flex items-center gap-2 text-red-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Immunohematology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-red-400" /> TRANS-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {TRANSFUSION_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-red-950/60 to-rose-950/60 border border-red-800/40">
            <h4 className="text-sm font-bold text-red-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Transfusion Medicine Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Bombay phenotype lacks H substance; produces potent complement-fixing <strong>Anti-H</strong>.</li>
              <li>MTP 1:1:1: Infuse <strong>10% Calcium Gluconate</strong> every 4 units to treat citrate hypocalcemia.</li>
              <li>TRALI vs TACO: TACO has <strong>hypertension, elevated BNP, and responds to Furosemide</strong>.</li>
              <li>RhoGAM 300 &mu;g neutralizes up to <strong>30 mL of fetal whole blood</strong> (add +1 safety vial).</li>
            </ul>
          </div>
        </div>
      </div>
