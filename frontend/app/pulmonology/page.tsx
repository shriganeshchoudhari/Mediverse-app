...
export default function PulmonologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("resp-spirometry-pft-flow-volume-curves");

  const activeModule = getPulmonologyModuleById(selectedModuleId) || PULMONOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/20 text-sky-300 border border-sky-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Pulmonology &amp; Respiratory Medicine Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🫁 Pulmonology &amp; Respiratory Medicine (RESP-401)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              PFT Spirometry &amp; Flow-Volume Curves (Asthma vs COPD vs ILD), NTEP Tuberculosis Diagnostics (GeneXpert CBNAAT &amp; BPaL MDR-TB Regimens), Light\'s Criteria Pleural Fluid Stratification &amp; Empyema Drainage, and Occupational Pneumoconioses &amp; OSA Polysomnography.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Pulmonology Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <PulmonologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-sky-950/40 border border-sky-800/60">
                <div className="flex items-center gap-2 text-sky-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Respiratory Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-sky-400" /> RESP-401 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {PULMONOLOGY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-sky-950/60 to-cyan-950/60 border border-sky-800/40">
            <h4 className="text-sm font-bold text-sky-300 mb-2 flex items-center gap-2">
              <Wind size={16} /> Respiratory Diagnostic Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Positive BD reversibility: <strong>&ge;12% &amp; &ge;200 mL</strong> increase in FEV1 or FVC.</li>
              <li>GeneXpert CBNAAT detects MTB and <strong>Rifampicin resistance (rpoB)</strong> in &lt;2 hours.</li>
              <li>Light\'s Criteria: Protein ratio &gt;0.5 or LDH ratio &gt;0.6 indicates <strong>Exudate</strong>.</li>
              <li>Mandatory chest tube drainage if pleural <strong>pH &lt; 7.20</strong> or glucose &lt; 40 mg/dL.</li>
            </ul>
          </div>
        </div>
      </div>
