...
export default function ClinicalOphthalmologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("ophthalmology-adv-angle-closure-glaucoma");

  const activeModule = getOphthalmologyAdvModuleById(selectedModuleId) || OPHTHALMOLOGY_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Ophthalmology &amp; Ocular Microsurgery Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              👁️ Advanced Ophthalmology &amp; Ocular Microsurgery (OPH-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Acute Angle-Closure Glaucoma (Pupillary block, fixed mid-dilated oval pupil, IOP 40-70 mmHg, multi-agent decompression with Timolol, Apraclonidine, IV Acetazolamide, Mannitol, Pilocarpine, and bilateral Laser Peripheral Iridotomy LPI), Retinal Vascular Emergencies (Central Retinal Artery Occlusion &apos;cherry-red spot&apos; and boxcarring vs Central Retinal Vein Occlusion &apos;blood and thunder&apos; with anti-VEGF therapy vs Rhegmatogenous Retinal Detachment with Shafer &apos;tobacco dust&apos; and pneumatic retinopexy), Uveitis &amp; Ocular Immunology (HLA-B27 acute anterior uveitis with sterile hypopyon and keratic precipitates, Toxoplasmosis &apos;headlight in the fog&apos; chorioretinitis, and CMV retinitis &apos;pizza-pie&apos; appearance in CD4 &lt;50/uL with Valganciclovir), and Corneal Ulcers &amp; Refractive Surgery (Pseudomonas contact lens bacterial keratitis, Herpes simplex dendritic ulcer with terminal bulbs and strict steroid contraindication, Fungal vegetative keratitis with Natamycin, Acanthamoeba pain out of proportion, and LASIK vs PRK).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <ClinicalOphthalmologyAdvLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Eye size={16} /> Ophthalmology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> OPH-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {OPHTHALMOLOGY_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-violet-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Clinical Ophthalmology Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Angle-Closure Glaucoma: <strong>Mid-dilated oval pupil + IOP &gt;40 &rarr; Bilateral LPI cure</strong>.</li>
              <li>CRAO Fundus: <strong>Diffuse retinal whitening with cherry-red foveal spot</strong>.</li>
              <li>HSV Keratitis: <strong>Branching dendritic ulcer; TOPICAL STEROIDS STRICTLY FORBIDDEN</strong>.</li>
              <li>CMV Retinitis: <strong>Occurs at CD4 &lt;50/uL; pizza-pie appearance treated with Valganciclovir</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
