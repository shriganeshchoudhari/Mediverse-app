...
export default function GeneticsLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("genetics-chromosomal-aneuploidies-rearrangements");

  const activeModule = getGeneticsModuleById(selectedModuleId) || GENETICS_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Medical Genetics &amp; Genomics Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🧬 Medical Genetics &amp; Genomics (GEN-201)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Chromosomal Aneuploidies (Trisomies 21, 18, 13, Turner &amp; Klinefelter), Mendelian &amp; Maternal Mitochondrial Heteroplasmy, Epigenetic Genomic Imprinting (15q11-q13 Prader-Willi vs Angelman), and Molecular Cytogenetics (CMA First-Tier &amp; ACMG Variant Classification).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Genetics Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <GeneticsLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Medical Genetics Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> GEN-201 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {GENETICS_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-violet-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Medical Genetics Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Down Syndrome: <strong>Endocardial cushion defect</strong> + duodenal atresia + AMKL (M7 AML) risk.</li>
              <li>Mitochondrial DNA: <strong>Maternal transmission only</strong> (0% transmission risk from affected fathers).</li>
              <li>Prader-Willi: Loss of active <strong>paternal 15q11-q13</strong> (70% deletion, 25% maternal UPD).</li>
              <li>Chromosomal Microarray (CMA): <strong>First-tier diagnostic test</strong> for developmental delay/autism.</li>
            </ul>
          </div>
        </div>
      </div>
