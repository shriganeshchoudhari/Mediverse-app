...
export default function AnesthesiologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("anes-airway-mallampati-algorithm");

  const activeModule = getAnesthesiologyModuleById(selectedModuleId) || ANESTHESIOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-500/20 text-teal-300 border border-teal-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Anesthesiology &amp; Critical Care Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              💨 Anesthesiology &amp; Critical Care (ANES-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Preoperative Airway Evaluation (Mallampati &amp; LEMON), ASA 2022 Difficult Airway CICO Cascade, Volatile MAC &amp; Malignant Hyperthermia, Neuromuscular Reversal (Sugammadex), and ARDSNet Lung-Protective Ventilation.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Anesthesiology Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <AnesthesiologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-teal-950/40 border border-teal-800/60">
                <div className="flex items-center gap-2 text-teal-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Anesthesia Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-teal-400" /> ANES-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {ANESTHESIOLOGY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-teal-950/60 to-emerald-950/60 border border-teal-800/40">
            <h4 className="text-sm font-bold text-teal-300 mb-2 flex items-center gap-2">
              <Wind size={16} /> Clinical Anesthesiology Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Execute <strong>Scalpel Cricothyroidotomy (eFONA)</strong> in CICO airway emergencies.</li>
              <li>Treat <strong>Malignant Hyperthermia</strong> immediately with IV Dantrolene 2.5 mg/kg.</li>
              <li>Encapsulate Rocuronium with <strong>Sugammadex</strong> for instant NMB reversal.</li>
              <li>Set ARDS tidal volume to <strong>4–8 mL/kg Predicted Body Weight</strong> (Pplat &le; 30 cmH2O).</li>
            </ul>
          </div>
        </div>
      </div>
