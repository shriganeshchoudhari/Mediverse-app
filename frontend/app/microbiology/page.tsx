...
export default function MicrobiologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("micr-immunology");

  const activeModule = getMicrobiologyModuleById(selectedModuleId) || MICROBIOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Microbiology & Immunology Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🦠 Medical Microbiology & Immunology (MICR-201)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Diagnostic bacteriology flowcharts (Catalase, Coagulase, Hemolysis, Oxidase), Coombs Hypersensitivity Types I–IV, bacterial toxin virulence engines, and Hepatitis B viral serology.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Link
              href="/exam"
              className="px-4 py-2 rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-95 text-white text-sm font-bold shadow-lg shadow-purple-500/20 transition"
            >
              Take Clinical Exam
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Microbiology Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <MicrobiologyLabViewer height="560px" />
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-purple-400" /> MICR-201 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {MICROBIOLOGY_CORE_MODULES.map((mod) => {
          ...
          {/* Quick Guide */}
          <div className="p-5 rounded-2xl bg-gradient-to-br from-purple-950/60 to-pink-950/60 border border-purple-800/40">
            <h4 className="text-sm font-bold text-purple-300 mb-2 flex items-center gap-2">
              <Bug size={16} /> Microbiology Lab Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Navigate <strong>Bacteriology Flowcharts</strong> to differentiate Gram-positive and Gram-negative organisms.</li>
              <li>Inspect <strong>Coombs Hypersensitivity Mechanisms</strong> (Types I–IV) and clinical antibody vs T-cell drivers.</li>
              <li>Examine <strong>Bacterial Exotoxins</strong> (ADP-ribosylation, superantigens, neurotoxins) and endotoxin shock.</li>
              <li>Master the <strong>Hepatitis B Serological Matrix</strong> and the pathognomonic window period marker.</li>
            </ul>
          </div>
        </div>
      </div>
