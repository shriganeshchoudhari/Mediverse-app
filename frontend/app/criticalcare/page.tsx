...
export default function CriticalCareLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("critical-care-adv-hemodynamics-oxygen-delivery");

  const activeModule = getCriticalCareAdvModuleById(selectedModuleId) || CRITICAL_CARE_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/20 text-sky-300 border border-sky-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Critical Care &amp; Hemodynamics Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🔬 Critical Care Medicine &amp; Hemodynamic Monitoring (CCM-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Advanced Hemodynamics &amp; Oxygen Delivery Dynamics (DO2, VO2, O2ER, Swan-Ganz PA catheter waveforms, Stewart-Hamilton thermodilution CO, ScvO2 vs SvO2), ARDS Mechanical Ventilation (Berlin definition mild/moderate/severe, ARDSNet 4-6 mL/kg PBW lung-protective ventilation, driving pressure &le; 14 cmH2O, prone positioning 16h/day PROSEVA, VV-ECMO), Dynamic Fluid Responsiveness &amp; Vasoactive Pharmacology (Passive Leg Raise PLR, Stroke Volume Variation SVV, Norepinephrine vs Vasopressin vs Dobutamine), and ICU Sedation, Delirium &amp; The ABCDEF Bundle (PADIS analgo-sedation, RASS score light sedation, Dexmedetomidine vs Benzodiazepines, CAM-ICU assessment, and the ABCDEF liberation bundle).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <CriticalCareLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-sky-950/40 border border-sky-800/60">
                <div className="flex items-center gap-2 text-sky-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Activity size={16} /> Critical Care Clinical Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-sky-400" /> CCM-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {CRITICAL_CARE_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-sky-950/60 to-cyan-950/60 border border-sky-800/40">
            <h4 className="text-sm font-bold text-sky-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Critical Care Pearls
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>ARDS Ventilation: <strong>4-6 mL/kg Predicted Body Weight PBW (Plateau &le; 30 cmH2O)</strong>.</li>
              <li>Prone Positioning: <strong>&ge;16 hours/day in moderate-severe ARDS (PaO2/FiO2 &lt; 150)</strong>.</li>
              <li>Dynamic Fluids: <strong>Passive Leg Raise &gt;10-15% increase in CO confirms responsiveness</strong>.</li>
              <li>Sedation: <strong>Target light sedation (RASS 0 to -1); avoid delirium-inducing benzodiazepines</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
