...
export default function NeurologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("neurology-adv-ischemic-stroke-vascular-localization");

  const activeModule = getNeurologyAdvModuleById(selectedModuleId) || NEUROLOGY_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Neurology Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🧠 Clinical Neurology &amp; Stroke Localization (NEURO-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Acute Ischemic Stroke Localization (MCA, ACA, PCA with macular sparing, Wallenberg PICA vs AICA lateral pontine), Intracranial Hemorrhages (Epidural MMA lucid interval vs Subdural bridging veins vs Subarachnoid thunderclap with Nimodipine), Demyelinating &amp; Movement Disorders (MS INO/oligoclonal vs GBS albuminocytologic dissociation; MG vs LEMS; Parkinson vs Huntington), and Spinal Cord Syndromes (Brown-S&eacute;quard, ASA, Syringomyelia, ALS).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <NeurologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Brain size={16} /> Clinical Neurology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> NEURO-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {NEUROLOGY_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-purple-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Neurology Diagnostic Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Wallenberg (PICA): <strong>Nucleus Ambiguus dysphagia/hoarseness + normal motor power</strong>.</li>
              <li>Epidural Bleed: <strong>Middle meningeal artery, lucid interval, lenticular CT (no suture crossing)</strong>.</li>
              <li>Guillain-Barr&eacute;: <strong>CSF Albuminocytologic Dissociation (high protein, normal WBC)</strong>.</li>
              <li>Syringomyelia: <strong>Bilateral 'cape-like' pain/temp loss with preserved dorsal columns</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
