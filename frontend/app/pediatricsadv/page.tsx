...
export default function PediatricsAdvLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("pediatrics-adv-congenital-heart-defects-shunts");

  const activeModule = getPediatricsAdvModuleById(selectedModuleId) || PEDIATRICS_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Pediatric Pathophysiology &amp; NICU Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              👶 Pediatric Pathophysiology &amp; Neonatal Intensive Care (PEDS-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Congenital Heart Defects (Cyanotic 5 Ts: ToF squatting reversal, TGA egg-on-a-string with PGE1, Truncus, TAPVR, Tricuspid Atresia vs Acyanotic VSD, ASD, PDA indomethacin, Coarctation), Neonatal Respiratory Distress (Surfactant deficiency RDS ground-glass CXR vs TTN vs Meconium Aspiration vs BPD), Pediatric GI Emergencies (Pyloric Stenosis hypochloremic alkalosis vs Intussusception air enema vs Midgut Volvulus Ladd surgery vs NEC), and Immunodeficiencies &amp; Inborn Errors (SCID absent thymus vs Bruton absent B cells vs Wiskott-Aldrich WATER triad; PKU mousy odor vs Galactosemia cataracts with E. coli sepsis).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <PediatricsAdvLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-emerald-950/40 border border-emerald-800/60">
                <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <HeartPulse size={16} /> Pediatric Clinical Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-emerald-400" /> PEDS-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {PEDIATRICS_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-emerald-950/60 to-teal-950/60 border border-emerald-800/40">
            <h4 className="text-sm font-bold text-emerald-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Pediatric Critical Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>TGA: <strong>Maintain ductal patency with PGE1 (Alprostadil) infusion immediately</strong>.</li>
              <li>Pyloric Stenosis: <strong>Correct hypochloremic alkalosis with IV Saline BEFORE surgery</strong>.</li>
              <li>Intussusception: <strong>Air/contrast enema is both diagnostic AND therapeutic in &gt;85%</strong>.</li>
              <li>Galactosemia: <strong>Bilateral oil-drop cataracts + fatal E. coli neonatal sepsis</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
