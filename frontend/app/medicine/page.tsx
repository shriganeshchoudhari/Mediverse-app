...
export default function MedicineLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("med-cardiology");

  const activeModule = getMedicineModuleById(selectedModuleId) || MEDICINE_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual General Medicine Clinical Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🩺 General Medicine & Therapeutics (MED-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              12-lead ECG STEMI infarct localizer, arterial blood gas Davenport nomogram engine, nephrology AKI KDIGO & FeNa calculator, and diabetic ketoacidosis (DKA) vs HHS resuscitation protocols.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Link
              href="/exam"
              className="px-4 py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-blue-600 hover:opacity-95 text-white text-sm font-bold shadow-lg shadow-indigo-500/20 transition"
            >
              Take Clinical Exam
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Medicine Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <MedicineLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Internal Medicine Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> MED-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {MEDICINE_CORE_MODULES.map((mod) => {
          ...
          {/* Quick Guide */}
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-blue-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Stethoscope size={16} /> Clinical Medicine Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Use the <strong>12-Lead ECG Localizer</strong> to map ST elevations to culprit arteries.</li>
              <li>Interpret <strong>Acid-Base disorders</strong> using Anion Gap & Winter's formula.</li>
              <li>Calculate <strong>Fractional Excretion of Sodium (FeNa)</strong> in acute kidney injury.</li>
              <li>Master the <strong>4 Pillars of HFrEF GDMT</strong> for mortality reduction.</li>
            </ul>
          </div>
        </div>
      </div>
