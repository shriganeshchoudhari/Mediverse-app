...
export default function HospitalAdminLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("hospital-admin-biomedical-waste-management-rules-2016");

  const activeModule = getHospitalAdminModuleById(selectedModuleId) || HOSPITAL_ADMIN_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Healthcare Quality &amp; Hospital Administration Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🏥 Hospital Administration (HADM-401)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Biomedical Waste Management (BMWM Rules 2016), Hospital Infection Control (CLABSI, VAP, CAUTI, SSI Bundles), Healthcare Quality Frameworks (Donabedian, NABH 5th Ed, JCI, BOR/ALOS Metrics), and Patient Safety (IPSG 1-6, WHO Surgical Checklist &amp; Ishikawa RCA).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Hospital Admin Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <HospitalAdminLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-blue-950/40 border border-blue-800/60">
                <div className="flex items-center gap-2 text-blue-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Building2 size={16} /> Hospital Administration Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-blue-400" /> HADM-401 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {HOSPITAL_ADMIN_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-blue-950/60 to-indigo-950/60 border border-blue-800/40">
            <h4 className="text-sm font-bold text-blue-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Healthcare Quality Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>BMWM: <strong>Yellow</strong> (Incineration), <strong>Red</strong> (Autoclave plastics), <strong>White</strong> (Sharps).</li>
              <li>CLABSI Bundle: <strong>Subclavian vein preferred</strong>; femoral strictly avoided.</li>
              <li>Patient Identifiers: At least <strong>2 identifiers</strong> (Name + UHID; NEVER room #).</li>
              <li>Bed Occupancy Rate (BOR): Optimal international benchmark is <strong>75% - 85%</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
