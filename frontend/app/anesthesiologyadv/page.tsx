...
export default function ClinicalAnesthesiologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("anesthesiology-adv-difficult-airway-cricothyroidotomy");

  const activeModule = getAnesthesiologyAdvModuleById(selectedModuleId) || ANESTHESIOLOGY_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Anesthesiology &amp; Perioperative Medicine Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🏥 Advanced Anesthesiology, Perioperative Medicine &amp; Pain Management (ANE-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Difficult Airway Algorithms (LEMON criteria, Mallampati Class I-IV, Cormack-Lehane grading, video laryngoscopy vs fiberoptic bronchoscope, and Front-of-Neck Access FONA Scalpel-Bougie-Tube Cricothyroidotomy in CICO emergencies), Local Anesthetic Systemic Toxicity LAST (Bupivacaine Nav1.5 cardiotoxicity, ASRA protocol, 20% Lipid Emulsion 1.5 mL/kg bolus + 0.25 mL/kg/min infusion, and avoiding Vasopressin/Lidocaine/Beta-blockers), Intravenous &amp; Inhalational Pharmacology (Propofol vs Etomidate 11-beta-hydroxylase adrenal suppression vs Ketamine dissociative NMDA; Malignant Hyperthermia RYR1 crisis with rapid EtCO2 surge and IV Dantrolene 2.5 mg/kg; PRIS high-dose syndrome), and Neuromuscular Blockade &amp; Monitoring (Succinylcholine Phase I/II block, hyperkalemia risk, and atypical pseudocholinesterase Dibucaine test; Cisatracurium Hofmann elimination; Train-of-Four TOF ratio &ge;0.90 extubation threshold; Sugammadex gamma-cyclodextrin encapsulation of Rocuronium at 2, 4, 16 mg/kg vs Neostigmine-Glycopyrrolate).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <ClinicalAnesthesiologyAdvLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Anesthesiology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> ANE-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {ANESTHESIOLOGY_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-violet-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Clinical Anesthesia Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>CICO Emergency: <strong>Scalpel-Bougie-Tube Cricothyroidotomy with size 6.0 cuffed tube</strong>.</li>
              <li>LAST Antidote: <strong>20% Lipid Emulsion 1.5 mL/kg bolus + 0.25 mL/kg/min infusion</strong>.</li>
              <li>Malignant Hyperthermia: <strong>EtCO2 surge &rarr; Stop volatiles + IV Dantrolene 2.5 mg/kg</strong>.</li>
              <li>Safe Extubation: <strong>Quantitative Train-of-Four (TOF) ratio &ge; 0.90 (90%)</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
