...
export default function PathologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("path-cell-injury");

  const activeModule = getPathologyModuleById(selectedModuleId) || PATHOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Pathology & Histopathology Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🔬 Pathology & Pathophysiology (PATH-201)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Virtual microscopy with multi-objective magnification, special histochemical stains, polarized light amyloidosis, cell injury patterns, neoplasia hallmarks, and chronological post-MI evolution.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Link
              href="/exam"
              className="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-600 to-orange-600 hover:opacity-95 text-white text-sm font-bold shadow-lg shadow-amber-500/20 transition"
            >
              Take Clinical Exam
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Pathology Slide Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <PathologySlideViewer height="560px" />
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-amber-400" /> PATH-201 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {PATHOLOGY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-950/60 to-orange-950/60 border border-amber-800/40">
            <h4 className="text-sm font-bold text-amber-300 mb-2 flex items-center gap-2">
              <Microscope size={16} /> Virtual Microscopy Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Toggle between <strong>4x</strong>, <strong>10x</strong>, <strong>40x</strong>, and <strong>100x Oil</strong> objectives.</li>
              <li>Switch between special histochemical stains (<strong>H&E</strong>, <strong>Masson</strong>, <strong>PAS</strong>, <strong>Congo Red</strong>, <strong>Prussian Blue</strong>, <strong>Ziehl-Neelsen</strong>).</li>
              <li>Activate <strong>Polarized Light</strong> on amyloid slides to observe <strong>Apple-Green Birefringence</strong>.</li>
              <li>Review the chronological post-MI evolution and mechanical rupture windows.</li>
            </ul>
          </div>
        </div>
      </div>
