...
export default function CardiovascularLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("cardiovascular-adv-wiggers-diagram-pv-loops");

  const activeModule = getCardiovascularAdvModuleById(selectedModuleId) || CARDIOVASCULAR_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Cardiovascular &amp; Hemodynamics Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🫀 Cardiovascular Pathophysiology (CARD-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Wiggers Diagram &amp; Valvular Pressure-Volume (PV) Loops (AS, AR, MS, MR), Jugular Venous Pressure (JVP) Waveforms &amp; Pericardial Dynamics, Invasive Shock Profiling (Swan-Ganz), and Advanced ACLS Resuscitation Algorithms.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <CardiovascularLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-rose-950/40 border border-rose-800/60">
                <div className="flex items-center gap-2 text-rose-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <HeartPulse size={16} /> Clinical Cardiovascular Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-rose-400" /> CARD-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {CARDIOVASCULAR_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-rose-950/60 to-pink-950/60 border border-rose-800/40">
            <h4 className="text-sm font-bold text-rose-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Advanced Hemodynamics Master Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Aortic Regurgitation: <strong>No isovolumetric phases</strong> (aortic valve never seals in diastole).</li>
              <li>Constriction vs Tamponade: Constriction has <strong>sharp deep y descent</strong>; Tamponade has <strong>blunted y descent</strong>.</li>
              <li>Cardiogenic Shock: <strong>High PCWP (&gt;18) + Low CI (&lt;2.0) + High SVR</strong>.</li>
              <li>Pulseless VF/VT: <strong>Immediate 200 J shock &rarr; CPR 2 min &rarr; Epinephrine &rarr; Amiodarone 300 mg</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
