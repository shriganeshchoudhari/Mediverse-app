...
export default function OncologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("oncology-cancer-biology-tnm-staging");

  const activeModule = getOncologyModuleById(selectedModuleId) || ONCOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/20 text-pink-300 border border-pink-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Oncology &amp; Palliative Radiotherapy Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🔬 Clinical Oncology &amp; Radiotherapy (ONCO-401)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Cancer Biology &amp; AJCC 8th Ed. TNM Staging, Cytotoxic Chemotherapy Antidotes &amp; Checkpoint Inhibitor irAEs, Radiobiology (The 4 Rs &amp; Linear-Quadratic Model), and Acute Oncologic Emergencies (SVC Syndrome, Spinal Cord Compression &amp; Tumor Lysis Syndrome).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Oncology Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <OncologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-pink-950/40 border border-pink-800/60">
                <div className="flex items-center gap-2 text-pink-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Oncology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-pink-400" /> ONCO-401 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {ONCOLOGY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-pink-950/60 to-rose-950/60 border border-pink-800/40">
            <h4 className="text-sm font-bold text-pink-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Clinical Oncology Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Mesna binds and neutralizes <strong>toxic acrolein</strong> from Cyclophosphamide.</li>
              <li>Malignant Spinal Cord Compression: <strong>Immediate IV Dexamethasone 16 mg</strong> + MRI spine within 24h.</li>
              <li>Tumor Lysis Syndrome: <strong>Rasburicase</strong> converts insoluble uric acid into soluble allantoin.</li>
              <li>Avoid radiation breaks: <strong>Accelerated clonogen repopulation</strong> begins ~28 days into treatment.</li>
            </ul>
          </div>
        </div>
      </div>
