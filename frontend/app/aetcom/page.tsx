...
export default function AetcomLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("aetcom-bioethics-principles-consent");

  const activeModule = getAetcomModuleById(selectedModuleId) || AETCOM_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual AETCOM &amp; Medical Ethics Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🤝 AETCOM &amp; Medical Ethics (AETCOM-001)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              The 4 Principles of Bioethics (Autonomy, Beneficence, Non-Maleficence, Justice), Informed Consent &amp; Decision-Making Capacity (CURB), The SPIKES Protocol for Delivering Bad News, Confidentiality &amp; Tarasoff Exceptions, Medical Malpractice (4 Ds), and End-of-Life Doctrine of Double Effect.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive AETCOM Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <AetcomLabViewer height="560px" />
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">
                  NMC AETCOM Module: {activeModule.unitCode}
                </span>
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-amber-950/40 border border-amber-800/60">
                <div className="flex items-center gap-2 text-amber-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Ethics Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-amber-400" /> AETCOM-001 Curriculum Modules
            </h3>

            <div className="flex flex-col gap-2.5">
              {AETCOM_CORE_MODULES.map((mod) => {
          ...
          {/* Quick Guide */}
          <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-950/60 to-yellow-950/60 border border-amber-800/40">
            <h4 className="text-sm font-bold text-amber-300 mb-2 flex items-center gap-2">
              <Scale size={16} /> Bioethics &amp; Communication Master Pearls
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>A competent adult has the absolute right of <strong>Autonomy</strong> to refuse treatment.</li>
              <li>Always fire a <strong>Warning Shot</strong> before delivering difficult medical diagnoses.</li>
              <li>Breach confidentiality under the <strong>Tarasoff Ruling</strong> to warn identifiable victims.</li>
              <li>The <strong>Doctrine of Double Effect</strong> ethically justifies aggressive palliative opioids.</li>
            </ul>
          </div>
        </div>
      </div>
