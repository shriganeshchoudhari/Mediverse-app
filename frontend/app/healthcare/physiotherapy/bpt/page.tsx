...
export default function BPTPage() {
  ...
  const currentYearData = BPT_CURRICULUM.find((y) => y.year === selectedYear) || BPT_CURRICULUM[0];

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
      {/* Hero Header */}
      <div className="border-b border-cyan-950/80 bg-gradient-to-r from-slate-950 via-cyan-950/40 to-slate-950 py-10 px-4 sm:px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <nav className="text-xs text-slate-400 mb-3 flex items-center gap-2">
              <Link href="/healthcare" className="text-cyan-400 hover:text-cyan-300 transition">
                Healthcare Landscape
              </Link>
              <span>/</span>
              <Link href="/healthcare/physiotherapy" className="text-cyan-400 hover:text-cyan-300 transition">
                Physiotherapy
              </Link>
              <span>/</span>
              <span className="text-slate-300">BPT Curriculum</span>
            </nav>

            <div className="flex items-center gap-2 mb-2">
              <span className="text-3xl sm:text-4xl">🦵</span>
              <div className="flex items-center gap-2">
                <span className="px-3 py-0.5 rounded-full text-xs font-extrabold uppercase tracking-wider bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                  {BPT_METADATA.regulatoryBody}
                </span>
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs font-bold">
                  {BPT_METADATA.duration}
                </span>
              </div>
            </div>

            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight mt-1">
              {BPT_METADATA.programName} ({BPT_METADATA.abbreviation})
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              4.5-year NCAHP/IAP competency-based curriculum covering musculoskeletal biomechanics, neuro-rehabilitation, cardiorespiratory physiotherapy, and electrotherapy.
            </p>
          </div>
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            >
              ...
              {showCompetencyMap ? 'Hide Competency Map' : 'View IAP Competencies'}
            </button>
            <Link
              href="/healthcare/physiotherapy"
              className="px-4 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-bold shadow-lg shadow-cyan-500/20 transition flex items-center gap-2"
            >
              <ArrowLeft size={16} /> Physio Portal
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Link
            href="/healthcare/physiotherapy/gait-analysis"
            className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-cyan-500/50 transition flex items-center gap-2 text-xs font-bold text-slate-200"
          >
            <span>🚶</span> Gait Analysis Lab
          </Link>
          <Link
            href="/healthcare/physiotherapy/joint-biomechanics"
            className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-cyan-500/50 transition flex items-center gap-2 text-xs font-bold text-slate-200"
          >
            <span>📐</span> Joint Biomechanics 3D
          </Link>
          <Link
            href="/healthcare/physiotherapy/neurorehab"
            className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-cyan-500/50 transition flex items-center gap-2 text-xs font-bold text-slate-200"
          >
            <span>🧠</span> Neuro-Rehab Simulator
          </Link>
          <Link
            href="/healthcare/physiotherapy/pulmonary-rehab"
            className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-cyan-500/50 transition flex items-center gap-2 text-xs font-bold text-slate-200"
          >
            <span>🫁</span> Pulmonary Rehab Protocols
          </Link>
        </div>
      </div>
        ...
        {showCompetencyMap && (
          <section className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl space-y-4">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <Award className="text-cyan-400" /> IAP / NCAHP Competency-Based Framework
            </h2>
            <IAPCompetencyMap />
          </section>
        )}
              ...
              >
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-slate-400">
                        Year {subj.year} • Credit Hours: {subj.creditHours}
                      </span>
                    </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                    <BookOpen size={14} className="text-cyan-400" /> Syllabus Chapters &amp; Movement Modules
                  </h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {subj.lessons.map((lesson, idx) => (
                      <div
                        key={lesson.id}
                        className="p-4 rounded-xl bg-slate-950/70 border border-slate-800/90 hover:border-cyan-500/50 transition-all flex flex-col justify-between group shadow-sm"
                      >
                        <div>
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-900 flex items-center justify-between gap-2">
                          <div className="flex gap-1.5 text-[10px]">
                            {lesson.hasSimulation && (
                              <span className="bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 px-1.5 py-0.5 rounded font-semibold">
                                Biomechanics Sim
                              </span>
                            )}
                            {lesson.isClinical && (
