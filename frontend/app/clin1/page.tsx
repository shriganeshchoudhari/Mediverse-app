...
export default function ClinicalClin1LabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("clin1-ward-rounds-soap-documentation");

  const activeModule = getClin1ModuleById(selectedModuleId) || CLIN1_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Postings I Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🏥 Clinical Postings I: Inpatient Medicine &amp; Bedside Rounds (CLIN-201)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Inpatient Ward Rounds &amp; SOAP Documentation (Morning presentation architecture with 24-hour vital ranges, strict I/Os, daily weight trends, and problem-based progress notes), Bedside Cardiovascular &amp; Hemodynamic Physical Examination (JVP measurement relative to sternal angle, waveform physics with cannon a waves in AV dissociation, giant holosystolic v waves in TR, Friedreich steep y descent in constrictive pericarditis vs blunted y descent in tamponade, Kussmaul sign, and pulsus paradoxus &gt;10 mmHg drop), Bedside Pulmonary, Abdominal &amp; Toxic-Metabolic Signs (Tactile fremitus, stony dullness, bronchial sounds, egophony, shifting dullness &gt;1,500 mL, fluid wave &gt;2,000 mL, asterixis negative myoclonus in hepatic encephalopathy/uremia/CO2 retention, and sustained ankle clonus), and Inpatient Fluid &amp; Electrolyte Resuscitation (Holiday-Segar 4-2-1 maintenance rule, normal saline hyperchloremia risk vs balanced crystalloids, hyponatremia safe correction limit &le;6-8 mEq/L/24h preventing osmotic demyelination, and 3-step emergency hyperkalemia membrane stabilization).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <ClinicalClin1LabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-emerald-950/40 border border-emerald-800/60">
                <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Inpatient Clinical Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-emerald-400" /> CLIN-201 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {CLIN1_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-emerald-950/60 to-teal-950/60 border border-emerald-800/40">
            <h4 className="text-sm font-bold text-emerald-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Inpatient Medicine Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Pulsus Paradoxus: <strong>Drop in SBP &gt; 10 mmHg during inspiration &rarr; Cardiac Tamponade</strong>.</li>
              <li>Asterixis Flap: <strong>Negative myoclonus of wrists &rarr; Hepatic encephalopathy, uremia, hypercapnia</strong>.</li>
              <li>Hyponatremia Limit: <strong>Correct &le; 6-8 mEq/L in 24h to prevent Osmotic Demyelination Syndrome</strong>.</li>
              <li>Hyperkalemia: <strong>Step 1 is IV Calcium Gluconate to stabilize cardiac membrane potential</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
