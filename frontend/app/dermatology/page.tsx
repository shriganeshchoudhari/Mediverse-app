...
export default function DermatologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("derm-psoriasis-papulosquamous");

  const activeModule = getDermatologyModuleById(selectedModuleId) || DERMATOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/20 text-pink-300 border border-pink-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Dermatology Clinical Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🧴 Dermatology &amp; Venereology (DERM-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Psoriasis vulgaris &amp; Auspitz sign, Bullous dermatoses (Pemphigus vs Pemphigoid &amp; Nikolsky sign), Malignant melanoma with Breslow depth invasion, and SJS / TEN SCORTEN emergency triage.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Dermatology Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <DermatologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-pink-950/40 border border-pink-800/60">
                <div className="flex items-center gap-2 text-pink-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Dermatology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-pink-400" /> DERM-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {DERMATOLOGY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-pink-950/60 to-rose-950/60 border border-pink-800/40">
            <h4 className="text-sm font-bold text-pink-300 mb-2 flex items-center gap-2">
              <Shield size={16} /> Clinical Dermatology Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Avoid <strong>Oral Systemic Corticosteroids</strong> in Psoriasis to prevent pustular transformation.</li>
              <li>Differentiate <strong>Nikolsky Positive Flaccid Bullae (Pemphigus)</strong> vs <strong>Nikolsky Negative Tense Bullae (Pemphigoid)</strong>.</li>
              <li>Measure <strong>Breslow Depth of Invasion</strong> in millimeters for melanoma prognosis.</li>
              <li>Transfer <strong>SJS / TEN ($&gt;30\%$ TBSA)</strong> immediately to specialized Burn Units.</li>
            </ul>
          </div>
        </div>
      </div>
