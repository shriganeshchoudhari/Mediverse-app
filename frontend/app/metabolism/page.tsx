...
export default function MetabolismLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("bioc-carbohydrate");

  const activeModule = getBiochemistryModuleById(selectedModuleId) || BIOCHEMISTRY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Biochemistry & Metabolism Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🧪 Medical Biochemistry (BIOC-101)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Interactive intermediary metabolism matrix (Glycolysis, Gluconeogenesis, TCA, Urea Cycle), lipoprotein transport cascade, and simulated inborn errors of metabolism.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Metabolic Pathway Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <MetabolicPathwayViewer height="560px" />
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-emerald-400" /> BIOC-101 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {BIOCHEMISTRY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-emerald-950/60 to-teal-950/60 border border-emerald-800/40">
            <h4 className="text-sm font-bold text-emerald-300 mb-2 flex items-center gap-2">
              <FlaskConical size={16} /> Metabolism Lab Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Switch between <strong>Well-Fed</strong>, <strong>Fasting</strong>, and <strong>Starvation</strong> states to visualize metabolic fluxes.</li>
              <li>Click <strong>Simulate Block</strong> on any enzyme to model inborn metabolic errors or drug inhibitions.</li>
              <li>Inspect <strong>Rate-Limiting Steps</strong>, allosteric modulators, and clinical disease correlations.</li>
              <li>Engage in <strong>Metabolic Quiz</strong> mode to solve patient presentation challenges.</li>
            </ul>
          </div>
        </div>
      </div>
