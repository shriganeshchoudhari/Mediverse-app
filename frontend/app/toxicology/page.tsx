...
export default function ToxicologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("toxicology-toxidromes-acute-poisoning-resuscitation");

  const activeModule = getToxicologyModuleById(selectedModuleId) || TOXICOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/20 text-red-300 border border-red-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Toxicology &amp; Poisoning Emergencies Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              💊 Clinical Toxicology (TOX-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              5 Core Clinical Toxidromes, Signature Overdoses &amp; Antidotes (APAP NAC, Salicylate NaHCO3, TCA NaHCO3, Digibind), Heavy Metal Chelation (Lead, Iron, Mercury), and Toxic Alcohols (Methanol, Ethylene Glycol &amp; Fomepizole) &amp; Enhanced Elimination.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Toxicology Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <ToxicologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-red-950/40 border border-red-800/60">
                <div className="flex items-center gap-2 text-red-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Toxicology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-red-400" /> TOX-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {TOXICOLOGY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-red-950/60 to-rose-950/60 border border-red-800/40">
            <h4 className="text-sm font-bold text-red-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Clinical Toxicology Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Organophosphates: Double <strong>Atropine</strong> q3-5m until pulmonary secretions dry.</li>
              <li>TCA Overdose: <strong>Hypertonic NaHCO3</strong> boluses to narrow QRS &lt;100 ms.</li>
              <li>Lead Encephalopathy: Give <strong>BAL first</strong> before CaNa2-EDTA to protect brain.</li>
              <li>Methanol/Ethylene Glycol: <strong>Fomepizole</strong> ADH inhibitor + emergent Hemodialysis.</li>
            </ul>
          </div>
        </div>
      </div>
