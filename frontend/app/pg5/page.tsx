...
export default function ClinicalPg5LabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("pg5-monochorionic-ttts-fetoscopic-laser");

  const activeModule = getPg5ModuleById(selectedModuleId) || PG5_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Postgraduate Obstetrics &amp; Fetal Medicine Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🏥 Postgraduate Advanced Obstetrics, Fetal Medicine &amp; Maternal Critical Care (PG-605)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Monochorionic Twin Gestations &amp; TTTS (Quintero Stages I-V, deep arteriovenous AV placental cotyledonary anastomoses, and Solomon fetoscopic selective laser photocoagulation along the vascular equator at 16-26 weeks gestation), Early-Onset Fetal Growth Restriction FGR (Longitudinal Doppler progression with Umbilical Artery absent/reversed end-diastolic flow AREDF, Middle Cerebral Artery brain-sparing vasodilation with CPR &lt;1.08, and Ductus Venosus reversed a-wave during atrial contraction triggering emergency delivery per the TRUFFLE trial), Amniotic Fluid Embolism AFE (Biphasic anaphylactoid collapse with Phase 1 acute pulmonary vasospasm and acute cor pulmonale / RV failure, Phase 2 LV dysfunction and fulminant consumptive DIC, and the A-OK resuscitation bundle of Atropine 1mg, Ondansetron 8mg, and Ketorolac 30mg IV alongside cryoprecipitate target fibrinogen &gt;200 mg/dL), and Resuscitative Hysterotomy / PMCD (Aortocaval compression physiology at &ge;20 weeks reducing venous return by 60%, the 4-minute decision-to-incision rule, the 5-minute complete fetal extraction standard, continuous manual left uterine displacement LUD, and immediate bedside laparotomy without transfer).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <ClinicalPg5LabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> MFM / Critical Care Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> PG-605 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {PG5_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-violet-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> MFM &amp; Obstetric Pearls
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>TTTS Laser: <strong>Solomon technique coagulates all AV anastomoses at 16-26w</strong>.</li>
              <li>FGR Delivery: <strong>Ductus Venosus reversed a-wave triggers emergency delivery (TRUFFLE)</strong>.</li>
              <li>AFE A-OK: <strong>Atropine 1mg, Ondansetron 8mg, Ketorolac 30mg IV given immediately</strong>.</li>
              <li>PMCD: <strong>Incise at 4 min of refractory arrest; extract fetus by 5 min to restore ROSC</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
