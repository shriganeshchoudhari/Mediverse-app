...
export default function InfectiousDiseasesLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("infectious-diseases-sepsis-definitions-surviving-sepsis-bundle");

  const activeModule = getInfectiousDiseaseModuleById(selectedModuleId) || INFECTIOUS_DISEASES_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Infectious Diseases &amp; Antimicrobial Stewardship Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🦠 Infectious Diseases (ID-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Sepsis-3 Definitions &amp; Surviving Sepsis Hour-1 Bundle, Fever of Unknown Origin (FUO) &amp; Tropical Fevers (Malaria, Dengue, Typhoid, Scrub Typhus), MDR ESKAPE Superbugs (MRSA, VRE, CRE), and WHO AWaRe Antimicrobial Stewardship.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive ID Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <InfectiousDiseaseLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-emerald-950/40 border border-emerald-800/60">
                <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Bug size={16} /> Clinical Infectious Disease Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-emerald-400" /> ID-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {INFECTIOUS_DISEASES_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-emerald-950/60 to-teal-950/60 border border-emerald-800/40">
            <h4 className="text-sm font-bold text-emerald-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Infectious Disease Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Sepsis Hour-1: <strong>30 mL/kg balanced crystalloids</strong> + Norepinephrine for MAP &ge;65.</li>
              <li>Severe Malaria: <strong>IV Artesunate</strong> is first-line; superior to quinine.</li>
              <li>MRSA Pneumonia: Daptomycin is <strong>contraindicated</strong> (surfactant binding); use Vancomycin/Linezolid.</li>
              <li>WHO AWaRe: <strong>&ge;60%</strong> of hospital antibiotic use must be from Access group.</li>
            </ul>
          </div>
        </div>
      </div>
