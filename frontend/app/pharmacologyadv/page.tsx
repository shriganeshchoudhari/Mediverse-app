...
export default function ClinicalPharmacologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("pharmacology-adv-tdm-kinetics");

  const activeModule = getPharmacologyAdvModuleById(selectedModuleId) || PHARMACOLOGY_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Pharmacology &amp; Rational Therapeutics Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🏥 Clinical Pharmacology &amp; Rational Therapeutics (PHARM-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Therapeutic Drug Monitoring &amp; Non-Linear Elimination Kinetics (Vancomycin AUC/MIC &ge; 400-600, Aminoglycoside once-daily peak/trough, Digoxin with DigiFab, Lithium, Phenytoin Michaelis-Menten zero-order saturation), Antimicrobial Stewardship &amp; Resistance (ESBL Meropenem, CRE Ceftazidime-avibactam, MRSA mecA/PBP2a Vancomycin vs Daptomycin inactivation by pulmonary surfactant vs Linezolid, Pseudomonas double coverage), Anticoagulation &amp; Direct Reversal Agents (DOACs vs Warfarin, Idarucizumab for Dabigatran, Andexanet alfa for FXa inhibitors, 4-Factor PCC + IV Vitamin K for Warfarin, Protamine for Heparin), and Chemotherapeutic Toxicities &amp; Rescue Antidotes (Doxorubicin cardiotoxicity with Dexrazoxane, Cisplatin nephro/ototoxicity with Amifostine, Cyclophosphamide acrolein cystitis with Mesna, Methotrexate Leucovorin rescue and Glucarpidase, Vincristine fatal intrathecal toxicity).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <ClinicalPharmacologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-purple-950/40 border border-purple-800/60">
                <div className="flex items-center gap-2 text-purple-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Pill size={16} /> Clinical Pharmacology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-purple-400" /> PHARM-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {PHARMACOLOGY_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-purple-950/60 to-fuchsia-950/60 border border-purple-800/40">
            <h4 className="text-sm font-bold text-purple-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Rational Therapeutics Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>MRSA Pneumonia: <strong>Daptomycin is inactivated by surfactant; use Linezolid or Vancomycin</strong>.</li>
              <li>Dabigatran Reversal: <strong>Idarucizumab 5g IV (humanized Fab decoy with 350x affinity)</strong>.</li>
              <li>Warfarin Urgent Reversal: <strong>4-Factor PCC (Kcentra) + IV Vitamin K</strong>.</li>
              <li>Cyclophosphamide Cystitis: <strong>Mesna (binds toxic metabolite Acrolein in bladder) + Hydration</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
