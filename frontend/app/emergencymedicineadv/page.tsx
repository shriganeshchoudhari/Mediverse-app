...
export default function EmergencyMedicineAdvLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("emergency-adv-acls-resuscitation-protocols");

  const activeModule = getEmergencyMedicineAdvModuleById(selectedModuleId) || EMERGENCY_MEDICINE_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-500/20 text-red-300 border border-red-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Emergency Medicine &amp; Resuscitation Science Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🚨 Emergency Medicine &amp; Resuscitation Science (EM-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Advanced Cardiac Life Support (ACLS shockable VF/pVT defib vs asystole/PEA epinephrine with H&apos;s and T&apos;s; TTM neuroprotection), Shock Differentiation (Cardiogenic high PCWP vs Hypovolemic low preload vs Distributive Septic vasoplegia with 1-hour bundle and norepinephrine vs Obstructive high CVP), Emergency Toxicology &amp; Toxidromes (Anticholinergic physostigmine vs Cholinergic SLUDGE atropine/2-PAM vs Opioid naloxone vs TCA sodium bicarbonate), and Acute Trauma Resuscitation (ATLS primary survey, tension pneumothorax needle thoracostomy, Beck triad tamponade pericardiocentesis, eFAST 6-view sonography, and 1:1:1 massive transfusion protocol with TXA).
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <EmergencyMedicineAdvLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-red-950/40 border border-red-800/60">
                <div className="flex items-center gap-2 text-red-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Flame size={16} /> Emergency Resuscitation Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-red-400" /> EM-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {EMERGENCY_MEDICINE_ADV_CORE_MODULES.map((mod) => {
          ...
          {/* Quick Guide */}
          <div className="p-5 rounded-2xl bg-gradient-to-br from-red-950/60 to-orange-950/60 border border-red-800/40">
            <h4 className="text-sm font-bold text-red-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Resuscitation Life Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Asystole/PEA: <strong>NEVER defibrillate; Epinephrine 1 mg IV ASAP + rule out H&apos;s and T&apos;s</strong>.</li>
              <li>Septic Shock: <strong>30 mL/kg crystalloids + Norepinephrine (target MAP &ge; 65 mmHg)</strong>.</li>
              <li>TCA Toxicity: <strong>IV Sodium Bicarbonate for QRS &gt; 100 ms or ventricular dysrhythmias</strong>.</li>
              <li>Tension PTX: <strong>Clinical diagnosis; immediate needle thoracostomy BEFORE CXR</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
