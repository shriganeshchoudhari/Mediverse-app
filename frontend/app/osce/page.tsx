        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Objective Structured Clinical Examination (OSCE) Stations
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🎯 Objective Structured Clinical Examination (OSCE-403)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Timed simulation stations covering Cardiovascular Precordial Auscultation &amp; Dynamic Murmurs, Cranial Nerves (II–XII) Systematic Exam, ATLS Trauma Primary Survey &amp; ACLS MegaCode Resuscitation, Suture Selection &amp; ABG Acid-Base Analysis, and SPIKES Breaking Bad News Communication.
            </p>
          </div>
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical OSCE Station Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> OSCE-403 Station Modules
            </h3>

            <div className="flex flex-col gap-2.5">
              {OSCE_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-blue-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Award size={16} /> OSCE Candidate Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Valsalva <strong>decreases AS/MR</strong> but <strong>increases HCM/MVP</strong> murmurs.</li>
              <li>Marcus Gunn pupil: Paradoxical dilation on <strong>swinging flashlight test</strong> (optic neuritis).</li>
              <li>Tension Pneumothorax: Immediate <strong>14G needle decompression</strong> in 5th ICS AAL.</li>
              <li>Winter's formula: Expected $PaCO_2 = 1.5[HCO_3^-] + 8 \pm 2$ for metabolic acidosis.</li>
            </ul>
          </div>
        </div>
      </div>
