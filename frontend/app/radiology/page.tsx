...
export default function RadiologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("rad-chest-radiography-abcde");

  const activeModule = getRadiologyModuleById(selectedModuleId) || RADIOLOGY_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/20 text-sky-300 border border-sky-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Radiodiagnosis &amp; Imaging Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              ☢️ Radiodiagnosis &amp; Imaging (RAD-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Systematic Chest Radiography (RIPE &amp; ABCDE), Non-Contrast Head CT in acute neurotrauma (EDH vs SDH vs SAH), Point-of-Care Trauma Ultrasound (E-FAST), and MRI Physics &amp; ALARA radiation protection.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Radiology Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <RadiologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-sky-950/40 border border-sky-800/60">
                <div className="flex items-center gap-2 text-sky-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Radiology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-sky-400" /> RAD-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {RADIOLOGY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-sky-950/60 to-blue-950/60 border border-sky-800/40">
            <h4 className="text-sm font-bold text-sky-300 mb-2 flex items-center gap-2">
              <Scan size={16} /> Clinical Radiology Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Recognize <strong>RML Pneumonia</strong> by loss of the right heart border (Silhouette Sign).</li>
              <li>Differentiate <strong>Epidural (biconvex, suture-limited)</strong> vs <strong>Subdural (crescentic, crosses sutures)</strong> hematomas.</li>
              <li>Detect hemoperitoneum in <strong>Morison\'s Pouch</strong> via FAST in unstable trauma.</li>
              <li>Apply the <strong>Inverse Square Law ($I \propto 1/d^2$)</strong> to minimize occupational radiation dose.</li>
            </ul>
          </div>
        </div>
      </div>
