...
export default function PmrLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("pmr-stroke-neuro-rehab-brunnstrom");

  const activeModule = getPmrModuleById(selectedModuleId) || PMR_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Physical Medicine &amp; Rehabilitation Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🩻 Physical Medicine &amp; Rehabilitation (PMR-401)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Stroke Neuro-Rehabilitation (Brunnstrom Stages 1–6 &amp; Modified Ashworth Scale), Spinal Cord Injury ASIA Impairment Scale (Grades A–E &amp; Autonomic Dysreflexia Emergency Protocol), Lower Limb Prosthetics (Medicare K-Levels K0–K4 &amp; AFO Prescriptions), and Gait Cycle Biomechanics &amp; Pathological Gaits.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive PMR Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <PmrLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-purple-950/40 border border-purple-800/60">
                <div className="flex items-center gap-2 text-purple-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical PMR Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-purple-400" /> PMR-401 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {PMR_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-purple-950/60 to-indigo-950/60 border border-purple-800/40">
            <h4 className="text-sm font-bold text-purple-300 mb-2 flex items-center gap-2">
              <Accessibility size={16} /> PMR Rehabilitation Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Brunnstrom Stage 3 represents <strong>peak spasticity</strong> with obligatory synergies.</li>
              <li>Autonomic Dysreflexia ($T_6$ lesions): <strong>Sit upright ($90^\circ$)</strong> and drain bladder.</li>
              <li>Dynamic response energy-storing carbon feet are indicated for <strong>K3 community ambulators</strong>.</li>
              <li>In Trendelenburg gait, hold cane in the <strong>contralateral (opposite) hand</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
