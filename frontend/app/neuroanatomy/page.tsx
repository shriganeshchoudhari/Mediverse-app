...
export default function ClinicalNeuroanatomyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("neuroanatomy-brainstem-stroke-syndromes");

  const activeModule = getNeuroanatomyModuleById(selectedModuleId) || NEUROANATOMY_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Neuroanatomy &amp; Localization Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🧠 Clinical Neuroanatomy (NEURO-201)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Brainstem Vascular Stroke Syndromes (Wallenberg, Dejerine, Weber, AICA), Spinal Cord Syndromes (Brown-Séquard, ASA Infarct, Syringomyelia, SCD), Higher Cortical Syndromes &amp; Aphasias (Broca, Wernicke, Gerstmann), and Cranial Nerve &amp; NMJ Localizers.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <ClinicalNeuroanatomyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-indigo-950/40 border border-indigo-800/60">
                <div className="flex items-center gap-2 text-indigo-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Brain size={16} /> Clinical Neurology Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-indigo-400" /> NEURO-201 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {NEUROANATOMY_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-violet-950/60 border border-indigo-800/40">
            <h4 className="text-sm font-bold text-indigo-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Clinical Neuro Localizer Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Wallenberg (PICA): Crossed sensory loss + Horner + dysphagia; <strong>Motor tract spared!</strong></li>
              <li>ASA Infarction: Bilateral paraplegia + pain/temp loss; <strong>Dorsal columns spared!</strong></li>
              <li>Syringomyelia: <strong>Cape-like</strong> bilateral loss of pain/temp (anterior white commissure).</li>
              <li>Gerstmann: Dominant angular gyrus (Agraphia, Acalculia, Finger agnosia, Left-right disorientation).</li>
            </ul>
          </div>
        </div>
      </div>
