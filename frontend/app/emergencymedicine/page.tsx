...
export default function EmergencyMedicineLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("em-acls-cardiac-arrest-algorithms");

  const activeModule = getEmergencyMedicineModuleById(selectedModuleId) || EMERGENCY_MEDICINE_CORE_MODULES[0];
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Emergency Medicine &amp; Resuscitation Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🧪 Emergency Medicine &amp; Resuscitation (EM-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              AHA ACLS Cardiac Arrest Algorithms (VF/pVT vs PEA/Asystole), Shock Classification &amp; Surviving Sepsis Bundle, Toxicology Toxidromes &amp; Rumack-Matthew Acetaminophen Nomogram, and ATLS Trauma Primary Survey &amp; Damage Control Resuscitation.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Left Column: Interactive Emergency Medicine Lab Viewer (8 Cols) */}
        <div className="lg:col-span-8 flex flex-col gap-6">
          <EmergencyMedicineLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-rose-950/40 border border-rose-800/60">
                <div className="flex items-center gap-2 text-rose-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <ShieldAlert size={16} /> Clinical Emergency Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-rose-400" /> EM-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {EMERGENCY_MEDICINE_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-rose-950/60 to-red-950/60 border border-rose-800/40">
            <h4 className="text-sm font-bold text-rose-300 mb-2 flex items-center gap-2">
              <Activity size={16} /> Emergency Resuscitation Master Guide
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>In shockable arrest (VF/pVT), deliver <strong>immediate 200J shock</strong> and resume CPR.</li>
              <li>Maintain <strong>MAP &ge; 65 mmHg</strong> with 30 mL/kg fluids &amp; Norepinephrine in sepsis.</li>
              <li>Inject <strong>IM Epinephrine 0.5 mg (1:1000)</strong> in mid-anterolateral thigh for anaphylaxis.</li>
              <li>Perform <strong>immediate needle decompression</strong> in Tension Pneumothorax; never wait for CXR!</li>
            </ul>
          </div>
        </div>
      </div>
