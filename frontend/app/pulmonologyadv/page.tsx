...
export default function PulmonologyAdvLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("pulmonology-adv-flow-volume-loops-pft");

  const activeModule = getPulmonologyAdvModuleById(selectedModuleId) || PULMONOLOGY_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-500/20 text-sky-300 border border-sky-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Pulmonary &amp; Critical Care Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              🫁 Pulmonary Pathophysiology &amp; Mechanical Ventilation (PULM-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Flow-Volume Loops &amp; PFTs (Obstructive vs Restrictive, Fixed/Variable Upper Airway), Mechanical Ventilation Mechanics (VCV, PCV, PIP vs Pplat, Static Compliance, Driving Pressure), ARDS Berlin Criteria &amp; Prone Rescue, and Hypoxemia Mechanisms &amp; Capnography.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <PulmonologyAdvLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-sky-950/40 border border-sky-800/60">
                <div className="flex items-center gap-2 text-sky-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Wind size={16} /> Clinical Critical Care Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-sky-400" /> PULM-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {PULMONOLOGY_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-sky-950/60 to-cyan-950/60 border border-sky-800/40">
            <h4 className="text-sm font-bold text-sky-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Critical Care Ventilation Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>High PIP + Normal Pplat (&le;30): <strong>Airway resistance problem</strong> (bronchospasm, secretions, kinked ETT).</li>
              <li>High PIP + High Pplat (&gt;30): <strong>Compliance problem</strong> (ARDS, Tension PTX).</li>
              <li>ARDSNet: <strong>Low tidal volume (6 mL/kg PBW) + Pplat &le;30 + Driving Pressure &lt;15</strong>.</li>
              <li>Prone Positioning: <strong>&ge;16 hours/day for severe ARDS (P/F &lt;150)</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
