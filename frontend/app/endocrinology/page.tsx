...
export default function EndocrinologyLabPage() {
  const [selectedModuleId, setSelectedModuleId] = useState<string>("endocrinology-adv-adrenal-disorders-cushing-conn");

  const activeModule = getEndocrinologyAdvModuleById(selectedModuleId) || ENDOCRINOLOGY_ADV_CORE_MODULES[0];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-bold uppercase tracking-wider mb-3">
              <Sparkles size={14} /> Virtual Clinical Endocrinology Laboratory
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
              ⚡ Clinical Endocrinology &amp; Metabolic Pathophysiology (ENDO-301)
            </h1>
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl mt-2 leading-relaxed">
              Adrenal Pathophysiology (Cushing high-dose DST, Addison Cosyntropin, Conn ARR &gt;20-30, Pheochromocytoma alpha-blockade), Thyroid Emergencies (Graves TSI vs Hashimoto TPO, 4-step Thyroid Storm protocol, Myxedema Coma), Calcium Homeostasis (PHPT vs FHH CCCR &lt;0.01 vs HHM PTHrP), and Multiple Endocrine Neoplasia (MEN 1, MEN 2A, MEN 2B) &amp; Diabetes Insipidus.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <EndocrinologyLabViewer height="560px" />
            ...
            {activeModule.clinicalVignettes.length > 0 && (
              <div className="mt-6 p-5 rounded-xl bg-amber-950/40 border border-amber-800/60">
                <div className="flex items-center gap-2 text-amber-400 text-xs font-bold uppercase tracking-wider mb-2">
                  <Zap size={16} /> Clinical Endocrinology Case Vignette
                </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-5">
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 shadow-xl">
            <h3 className="text-base font-bold text-white mb-3 flex items-center gap-2">
              <BookOpen size={18} className="text-amber-400" /> ENDO-301 Curriculum Units
            </h3>

            <div className="flex flex-col gap-2.5">
              {ENDOCRINOLOGY_ADV_CORE_MODULES.map((mod) => {
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-950/60 to-orange-950/60 border border-amber-800/40">
            <h4 className="text-sm font-bold text-amber-300 mb-2 flex items-center gap-2">
              <Zap size={16} /> Endocrinology Diagnostic Rules
            </h4>
            <ul className="text-xs text-slate-300 space-y-2 list-disc list-inside">
              <li>Pheochromocytoma: <strong>Alpha-blocker 10-14 days BEFORE beta-blocker</strong>.</li>
              <li>Thyroid Storm: <strong>Lugol iodine delayed &ge;1h AFTER thionamide</strong>.</li>
              <li>FHH vs PHPT: <strong>CCCR &lt;0.01 in FHH (Benign: DO NOT OPERATE!)</strong>.</li>
              <li>MEN 1: <strong>3 Ps: Pituitary, Parathyroid 95%, Pancreatic islet tumors</strong>.</li>
            </ul>
          </div>
        </div>
      </div>
