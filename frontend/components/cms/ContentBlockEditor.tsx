...
const BLOCK_TYPES = [
  ...
  { value: 'CLINICAL_CASE', label: '🩺 Clinical Case', desc: 'Patient vignette & labs' },
  { value: 'RX_CARD', label: '💊 Prescribing Rx Card', desc: 'Evidence-based formulary' },
  { value: 'DECISION_TREE', label: '🔀 Decision Pathway', desc: 'Algorithmic clinical branching' },
...
export default function ContentBlockEditor({ block, lessonId, onSave, onCancel }: ContentBlockEditorProps) {
  ...
  const placeholders: Record<string, string> = {
    ...
    CLINICAL_CASE: '{\n  "patientProfile": { "age": 54, "gender": "Male", "chiefComplaint": "Crushing chest pain" },\n  "vitals": { "bp": "142/90", "hr": "104" },\n  "decisionQuestions": []\n}',
    RX_CARD: '{\n  "condition": "Acute Bacterial Meningitis",\n  "firstLineRegimen": [{ "drugName": "Ceftriaxone", "dosage": "2g IV Q12H" }]\n}',
    DECISION_TREE: '{\n  "title": "Triage Pathway",\n  "initialNodeId": "node-1",\n  "nodes": []\n}',
    DIAGRAM: '```mermaid\ngraph TD\n  A[Start] --> B{Condition}\n  B -->|Yes| C[Result A]\n  B -->|No| D[Result B]\n```',
