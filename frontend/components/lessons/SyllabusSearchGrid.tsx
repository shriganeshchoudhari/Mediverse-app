...
const DOMAIN_TABS = [
  ...
  { id: 'veterinary', label: '🐾 Vet' },
  { id: 'public-health', label: '🌍 Pub.Health' },
];
...
export default function SyllabusSearchGrid() {
  ...
  return (
    <div className="space-y-8">
      ...
      {activeDomainTab !== 'allopathic' && (
        <div className={styles.nonMbbsBanner}>
          <div className={styles.nonMbbsTitle}>
            {DOMAIN_TABS.find(t => t.id === activeDomainTab)?.label} Curriculum
          </div>
          <p>Explore the full {DOMAIN_TABS.find(t => t.id === activeDomainTab)?.label.split(' ').slice(1).join(' ')} curriculum and simulators in the dedicated domain portal.</p>
          <Link href={`/healthcare/${activeDomainTab}`} className={styles.nonMbbsLink}>
            Open {DOMAIN_TABS.find(t => t.id === activeDomainTab)?.label} Portal →
          </Link>
        </div>
      )}
